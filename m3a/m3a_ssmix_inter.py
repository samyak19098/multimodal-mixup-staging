	...
	for epoch in range(epochs):
		for idx, (text, audio, pos, speak, label) in enumerate(train_set):
			with tf.GradientTape() as super_tape:
				...
				# audio_mixed = inter_mix(audio.numpy()[:3,-10:, :2], audio.numpy()[:3,-10:, :2][permutation], saliency_audio[:3,-10:, :2], saliency_audio[:3,-10:, :2][permutation], 0.3)
				# text_mixed = inter_mix(text.numpy()[:3,-10:, :2], text.numpy()[:3,-10:, :2][permutation], saliency_text[:3,-10:, :2], saliency_text[:3,-10:, :2][permutation], 0.3)
				# s = input()
				...
				loss_value = 0.8 * loss_value_1 + 0.2 * loss_value_2
